...
app.post("/accountCreationSSE", async (req, res) => {
  ...
  try {
    ...
    const accountData = {
      ...
      snapshotId: "6P0ooLYgMTnbMXCwigVB",
    };
    ...
    const password = `Marketcl!ng_${country}${postalCode}`;
    const userPayload = {
    ...
    // ==============================================================
    // Step 6: Create Google Drive Folder BEFORE Updating Custom Values
    // ==============================================================
    console.log(
      "Step 6: Creating Google Drive folder before updating custom values."
    );
    sendEvent(
    ...
    // Construct the Drive folder link using the folder ID.
    const driveFolderLink = `https://drive.google.com/drive/folders/${folderId}`;
    console.log("Drive Folder Link:", driveFolderLink);

    // ==============================================================
    // Step 7: Update Custom Values (Including Client Assets Folder Link)
    // ==============================================================
    ...
    const fieldsToSync = [
      ...
      "Client Assets Folder Link", // new custom field for Drive folder link
    ];
    ...
    for (let fieldName of fieldsToSync) {
      if (fieldName !== "Client Assets Folder Link") {
    ...
    // Update "Client Assets Folder Link" custom field with the Drive folder link.
    try {
      const clientAssetsField = childCustom["Client Assets Folder Link"];
      if (clientAssetsField) {
        const updateUrl = `${process.env.GHL_API_DOMAIN}/locations/${creationResponse.id}/customValues/${clientAssetsField.id}`;
        const updatePayload = {
          name: "Client Assets Folder Link",
          value: driveFolderLink,
        };
        console.log(
          `Updating "Client Assets Folder Link" with link: ${driveFolderLink}`
        );
        await axios.put(updateUrl, updatePayload, {
          headers: {
            Accept: "application/json",
        ...
        console.log(`Updated "Client Assets Folder Link" successfully.`);
      } else {
      ...
      console.error(
        ...
        clientAssetsError.response
          ? clientAssetsError.response.data
